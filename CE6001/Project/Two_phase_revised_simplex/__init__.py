...
class two_pahse_revised_simplex():
    def __init__(self): 
        self.A = []
        self.b = []
        self.c = []
    ...
    def phase_one(self, table):
        artificial_var = np.copy(self.base)
        check_phase_one = True
        while check_phase_one:
            self.find_pivot(table)
            if any(element in self.base for element in artificial_var):
                check_phase_one = True
            else:
                self.non_base = np.setdiff1d(self.non_base, artificial_var)
                check_phase_one = False
    ...
    def init_table(self):
        ...
        # Integrate artifical variables
        w = np.ones((1, len(self.b)))
        new_w = np.zeros((1, table_AB.shape[1]))
        new_w[ 0,len(self.c):len(self.c)+len(self.b)] = w
        phase_one_table = np.concatenate((table_AB, new_w), axis =0)
        ...
        for i in range(len(self.b)):
            self.base[i] = str(len(self.c) + i)
        for j in range(num_variables - len(self.base)):
            self.non_base.append(str(j))

        # Find basic feasible variables in Phase I
        self.phase_one(phase_one_table)
